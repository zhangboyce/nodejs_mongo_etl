...
ExportProject.prototype.execute = function(callback) {
    ...
    function* insertProjects(items) {
        ...
        _.each(items, item => {
            ...
            importBatch.find({id: _item.id}).upsert().updateOne(_item);
        });

        if (importBatch.length != 0) {
            let executeResult = yield importBatch.execute();
            console.log(`> Inserted ${items.length} ${this.type} projects.`);
    ...
    function* insertProjectTexts(items) {
        ...
        if (projectTextBatch.length != 0) {
            ...
            console.log(`> Inserted ${this.type} project text result: ${JSON.stringify(executeResult)}`);
        }
    }
};
